 ...
 */
gboolean
gst_ce_meta_init (GstMeta * meta, gpointer params, GstBuffer * buffer)
{
  gint32 phys;
  GstMapInfo info;
  ...
  Memory_registerContigBuf (info.data, info.size, phys);
  cemeta->addr = info.data;
 ...
 */
void
gst_ce_meta_free (GstMeta * meta, GstBuffer * buffer)
{
  GstCEMeta *cemeta;
...
const GstMetaInfo *
gst_ce_meta_get_info (void)
{
  static const GstMetaInfo *meta_info = NULL;

  if (g_once_init_enter (&meta_info)) {
    const GstMetaInfo *meta =
        gst_meta_register (gst_ce_meta_api_get_type (), "GstCEMeta",
        sizeof (GstCEMeta), (GstMetaInitFunction) gst_ce_meta_init,
        (GstMetaFreeFunction) gst_ce_meta_free,
