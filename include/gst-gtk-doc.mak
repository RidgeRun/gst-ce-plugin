...
CLEANFILES = $(SCANOBJ_FILES_0) $(REPORT_FILES) $(DOC_STAMPS) doc-registry.xml
...
setup-build.stamp: $(content_files)
	-@if test "$(abs_srcdir)" != "$(abs_builddir)" ; then \
	    ...
	    files=`echo $(DOC_MAIN_SGML_FILE) $(DOC_OVERRIDES) $(DOC_MODULE)-sections.txt $(DOC_MODULE).types $(content_files) $(SCANOBJ_FILES)`; \
...
# clean files copied for nonsrcdir templates build
	@if test x"$(srcdir)" != x. ; then \
	        rm -rf $(SCANOBJ_FILES) $(DOC_MODULE).types $(MAINTAINER_DOC_STAMPS); \
	fi
else
all-local:
clean-local-gtkdoc:
